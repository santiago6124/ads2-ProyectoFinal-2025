	...
	pipeline := []bson.M{
		{
			"$group": bson.M{
				...
				"positive_pnl":    bson.M{"$sum": bson.M{"$cond": []interface{}{bson.M{"$gt": []string{"$profit_loss", 0}}, 1, 0}}},
				"negative_pnl":    bson.M{"$sum": bson.M{"$cond": []interface{}{bson.M{"$lt": []string{"$profit_loss", 0}}, 1, 0}}},
				"neutral_pnl":     bson.M{"$sum": bson.M{"$cond": []interface{}{bson.M{"$eq": []string{"$profit_loss", 0}}, 1, 0}}},
				"high_growth":     bson.M{"$sum": bson.M{"$cond": []interface{}{bson.M{"$gt": []string{"$profit_loss_percentage", 0.2}}, 1, 0}}},
			},
		},
	}
